...
class UTFTGLUE : public TFT_kbv
{
    public:
//  UTFTGLUE() : TFT_kbv() {}
    UTFTGLUE(int model_ID, int RS, int WR,int CS, int RST, int RD = A0)
             : TFT_kbv(CS, RS, WR, RD, RST) { _model_ID = model_ID; }
    void InitLCD(byte orientation=LANDSCAPE) {
        TFT_kbv::reset();
        uint16_t ID = TFT_kbv::readID();
...
//         if (ID == 0x9327 && orientation == LANDSCAPE) orientation = 3;
        TFT_kbv::begin(ID);
        TFT_kbv::setRotation(_orient = orientation);
        _radius = 4;
    }
    void clrScr() { TFT_kbv::fillScreen(0x0000);}
    void drawPixel(int x, int y) { TFT_kbv::drawPixel(x, y, _fcolor);}
    void drawLine(int x1, int y1, int x2, int y2) { TFT_kbv::drawLine(x1, y1, x2, y2, _fcolor);}
    void fillScr(uint16_t color) { TFT_kbv::fillScreen(color);}
    void fillScr(byte r, byte g, byte b) { TFT_kbv::fillScreen(setrgb(r, g, b));}
    void drawRect(int x1, int y1, int x2, int y2) {
        int w = x2 - x1 + 1, h = y2 - y1 + 1;
        if (w < 0) { x1 = x2; w = -w; }
        if (h < 0) { y1 = y2; h = -h; }
        TFT_kbv::drawRect(x1, y1, w, h, _fcolor);
    }
    void drawRoundRect(int x1, int y1, int x2, int y2) {
        int w = x2 - x1 + 1, h = y2 - y1 + 1;
        if (w < 0) { x1 = x2; w = -w; }
        if (h < 0) { y1 = y2; h = -h; }
        TFT_kbv::drawRoundRect(x1, y1, w, h, _radius, _fcolor);
    }
    void fillRect(int x1, int y1, int x2, int y2) {
        int w = x2 - x1 + 1, h = y2 - y1 + 1;
        if (w < 0) { x1 = x2; w = -w; }
        if (h < 0) { y1 = y2; h = -h; }
        TFT_kbv::fillRect(x1, y1, w, h, _fcolor);
    }
    void fillRoundRect(int x1, int y1, int x2, int y2) {
        int w = x2 - x1 + 1, h = y2 - y1 + 1;
        if (w < 0) { x1 = x2; w = -w; }
        if (h < 0) { y1 = y2; h = -h; }
        TFT_kbv::fillRoundRect(x1, y1, w, h, _radius, _fcolor);
    }
    void drawCircle(int x, int y, int radius) { TFT_kbv::drawCircle(x, y, radius, _fcolor);}
    void fillCircle(int x, int y, int radius) { TFT_kbv::fillCircle(x, y, radius, _fcolor);}
    void setColor(byte r, byte g, byte b)  { setColor(setrgb(r, g, b));}
    void setColor(uint16_t c)  { TFT_kbv::setTextColor(_fcolor = c, _bcolor); }
    ...
    void setBackColor(uint32_t c)  { 
        ...
        TFT_kbv::setTextColor(_fcolor, _bcolor);
    }
    void print(const char *st, int x, int y, int deg=0)  {
        settextcursor((char*)st, x, y); TFT_kbv::print(st);}
    void print(char *st, int x, int y, int deg=0)  {
        settextcursor(st, x, y); TFT_kbv::print(st);}
    void print(String st, int x, int y, int deg=0) {
        settextcursor((char*)(st.c_str()), x, y); TFT_kbv::print(st);}
    ...
    void printNumF(double num, byte dec, int x, int y, char divider='.', int length=0, char filler=' ') {
        ...
        settextcursor(buf, x, y, length * _dig_wid * TFT_kbv::textsize_x); 
        TFT_kbv::print(buf);
    }
    void setTextSize(int sz) { TFT_kbv::setTextSize(gfxFont == NULL ? sz : 1); } // ####### GFX ########
    void setFont(const GFXfont* font) {
        TFT_kbv::setFont(font);
        _ascend = 8;
        _descend = 1;
        _dig_wid = 6;
        if (font != NULL) {
            int16_t x = 0, y = 0, x1, y1 = 0, x2, y2 = 0;
            TFT_kbv::setTextSize(1);
    ...
    void drawBitmap(int x, int y, int sx, int sy, const uint16_t *data, int scale=1) {
        uint16_t color;
        TFT_kbv::setAddrWindow(x, y, x + sx*scale - 1, y + sy*scale - 1);
        if (scale == 1) TFT_kbv::pushColors((const uint8_t*)data, sx * sy, 1);
        else {
            for (int row = 0; row < sy; row++) {
                for (int col = 0; col < sx; col++) {
                    color = pgm_read_word(data + (row*sx + col)*1);
                    TFT_kbv::fillRect(x+col*scale, y+row*scale, scale, scale, color);
                }
            }
        }
    }
...
//  void lcdOn();
    ...
    int  getDisplayXSize() { return TFT_kbv::width(); }
    int  getDisplayYSize() { return TFT_kbv::height(); }
...
//    uint8_t _ascend, _descend, _dig_wid;
    ...
    void settextcursor(char *st, int x, int y, int pad = 0) {
        ...
        if (is_gfx) {    //only GFX fonts need to paint background
            ...
            if (_fcolor != _bcolor) TFT_kbv::fillRect(x, y, len + pad + 1, h, _bcolor);
            y += h;
        }
        TFT_kbv::setCursor(x + pad, y);
    }
