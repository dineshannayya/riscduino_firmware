...
#include <TFT_kbv.h> // Hardware-specific library
#if !defined(AVR)
...
class TFT_HX8357GLUE : public TFT_kbv
{
    public:
        TFT_HX8357GLUE() {}
        void begin(void) {
		...
		void init(void)
        {
            ...
            if (_ID == 0x00D3 || _ID == 0xD3D3)
			   _ID = 0x9486;
            TFT_kbv::begin(_ID);
            TFT_kbv::setRotation(1);
            _first = true;
        }
        void setWindow(int16_t x0, int16_t y0, int16_t x1, int16_t y1)
        {
            TFT_kbv::setAddrWindow(x0, y0, x1, y1);
            _first = true;
        }

        void pushColors(uint8_t *data, uint16_t len)
        {
            TFT_kbv::pushColors((uint8_t*)data, len, _first);
            _first = false;
        }

        void pushColors(uint16_t *data, uint8_t len)
        {
            TFT_kbv::pushColors((uint16_t*)data, len, _first);
            _first = false;
        }

        void pushColors(uint16_t *data, uint16_t len, int first)
        {
            TFT_kbv::pushColors((uint16_t*)data, len, first);
        }

        void pushColor(uint16_t color)
        {
            uint16_t c = color;
			TFT_kbv::pushColors(&c, 1, _first);
            _first = false;
        }
        ...
        void setCursor(int16_t x, int16_t y, uint8_t idx)
        {
            ...
            TFT_kbv::setFont(f);
//            TFT_kbv::setCursor(x, y + f->glyph->height);

            TFT_kbv::setCursor(x, y);
        }
        
        void setTextFont(uint8_t font)
        {
            _font = font;
            TFT_kbv::setFont(Fonts[_font]);
        }
