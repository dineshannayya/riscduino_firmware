...
TFT_kbv::TFT_kbv(int CS, int RS, int WR, int RD, int _RST):Adafruit_GFX(240, 320)
{
...
void TFT_kbv::WriteCmdData(uint16_t cmd, uint16_t dat) { writecmddata(cmd, dat); }
...
void TFT_kbv::pushCommand(uint16_t cmd, uint8_t * block, int8_t N) { WriteCmdParamN(cmd, N, block); }
...
uint16_t TFT_kbv::readReg(uint16_t reg, int8_t index)
{
    uint16_t ret;
    uint8_t lo;
    if (!done_reset)
...
uint32_t TFT_kbv::readReg32(uint16_t reg)
{
...
uint32_t TFT_kbv::readReg40(uint16_t reg)
{
    uint16_t h = readReg(reg, 0);
...
int16_t TFT_kbv::readGRAM(int16_t x, int16_t y, uint16_t * block, int16_t w, int16_t h)
{
...
void TFT_kbv::setRotation(uint8_t r)
{
...
void TFT_kbv::drawPixel(int16_t x, int16_t y, uint16_t color)
{
...
void TFT_kbv::setAddrWindow(int16_t x, int16_t y, int16_t x1, int16_t y1)
{
#if defined(OFFSET_9327)
	if (_lcd_ID == 0x9327) {
...
void TFT_kbv::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    int16_t end;
#if defined(SUPPORT_9488_555)
    if (is555) color = color565_to_555(color);
...
void TFT_kbv::pushColors(uint16_t * block, int16_t n, bool first)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, 0);
}
void TFT_kbv::pushColors(uint8_t * block, int16_t n, bool first)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, 2);   //regular bigend
}
void TFT_kbv::pushColors(const uint8_t * block, int16_t n, bool first, bool bigend)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, bigend ? 3 : 1);
}

void TFT_kbv::vertScroll(int16_t top, int16_t scrollines, int16_t offset)
{
#if defined(OFFSET_9327)
	if (_lcd_ID == 0x9327) {
...
void TFT_kbv::invertDisplay(bool i)
{
    uint8_t val;
...
#define TFT_kbvLCD_DELAY 0xFFFF
#define TFT_kbvLCD_DELAY8 0x7F
...
void TFT_kbv::begin(uint16_t ID)
{
...
case 0x4532:    // thanks Leodino
    ...
    case 0x5310:
        ...
        static const uint8_t NT35310_regValues[] PROGMEM = {        //
            TFT_kbvLCD_DELAY8, 10,    //just some dummy
        };
...
//            (0x70), 1, 0x05,    //Panel [06] 16-bit
    ...
    case 0x9090:                //BIG CHANGE: HX8357-D was 0x8357
      ...
      common_8357:
        static const uint8_t HX8357C_regValues[] PROGMEM = {
            TFT_kbvLCD_DELAY8, 1,  //dummy table
        };
    ...
    case 0x8230:    //thanks Auzman
        ...
        static const uint16_t UC8230_regValues[] PROGMEM = {
            ...
            TFT_kbvLCD_DELAY, 100, //at least 100ms
            ...
            TFT_kbvLCD_DELAY, 10, //at least 10ms
            ...
            0x0000, 0x0001,
            TFT_kbvLCD_DELAY, 200, // Delay 200 ms
    ...
    case 0x9335:
      ...
      common_93x5:
        static const uint16_t ILI9325_regValues[] PROGMEM = {
            ...
            0x0007, 0x0001,
            TFT_kbvLCD_DELAY, 200,  // Dis-charge capacitor power voltage
            0x0010, 0x1690,     // SAP=1, BT=6, APE=1, AP=1, DSTB=0, SLP=0, STB=0
            0x0011, 0x0227,     // DC1=2, DC0=2, VC=7
            TFT_kbvLCD_DELAY, 50,   // wait_ms 50ms
...
//*************Power On sequence ****************//
        ...
        TFT_kbvLCD_DELAY, 50,   // Delay 50ms
...
#define GAMMA9486 4
    ...
    if (table8_ads != NULL) {
        static const uint8_t reset_off[] PROGMEM = {
            0x01, 0,            //Soft Reset
            TFT_kbvLCD_DELAY8, 150,  // .kbv will power up with ONLY reset, sleep out, display on
            0x28, 0,            //Display Off
