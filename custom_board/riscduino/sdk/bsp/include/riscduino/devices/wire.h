...
#define WIRE_CTRL_IACK(x)          ((x) & 0x1)          // Expect ACK 
#define WIRE_CTRL_ACK(x)           (((x) & 0x1) << 3)   // Transmit Act
#define WIRE_CTRL_WR(x)            (((x) & 0x1) << 4)   // Write
#define WIRE_CTRL_RD(x)            (((x) & 0x1) << 5)   // READ
#define WIRE_CTRL_STOP(x)          (((x) & 0x1) << 6)   // STOP
#define WIRE_CTRL_START(x)         (((x) & 0x1) << 7)   // START

#define WIRE_CMD_ENB(x)            (((x) & 0x1) << 7)   // Core Enable
#define WIRE_CMD_IEN(x)            (((x) & 0x1) << 6)   // I2C Interrupt Enabled
